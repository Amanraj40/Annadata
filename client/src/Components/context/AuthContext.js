...
//his is a functional component that serves as the provider for the AuthContext.
function AuthContextProvider(props) {
  ...
  async function getLoggedIn() {
    ...
    const loggedInRes = await axios.get("http://localhost:3177/auth/loggedIn", { withCredentials: true });//The request is made with credentials ({ withCredentials: true }), which allows sending cookies for authentication.
...
//Other components in the application can consume the context values (loggedIn, user, getLoggedIn) using the useContext hook or by wrapping components with AuthContextProvider.
