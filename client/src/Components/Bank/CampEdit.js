...
const CampEdit = (props) => {
   ...
   const [flag, setFlag] = useState(true); // flag to manage some conditional rendering (currently not used in the component)
   const [popup, setPopup] = useState(-1); // Manages the visibility of the popup (-1 means hidden)
   const [sent, setSent] = useState([]); // Holds data for sending information (like selected donors)
                ...
                //j: Represents the index (position) of the current donor in the array. Although it is not used in this code, it is available if needed.
                                props.data.donors.map((k, j) => {
                                    return (
                                        ...
                                        <CampsCheck 
                                        setSent={setSent} // Allows CampsCheck to update the sent state in the parent component (CampEdit).
                                        popup={popup} // Allows CampsCheck to know the current state of the popup (whether it is open or closed).
                                        setPopup={setPopup} // Allows CampsCheck to change the popup state (e.g., to open or close the popup).
                                        data={k} //Provides CampsCheck with the data it needs about the current donor.
                                        camp={props.data._id} // Provides CampsCheck with the ID of the camp, which might be needed for operations related to the camp.
                                    />
                                    )
                                })
                            }
